...
function 초기설정실행() {
  ...
  // 마감예정일 = 신청일 + 15일 수식 (데이터 행 전체)
  const 신청열문자 = columnLetter(iD신청);
  일정시트.getRange(2, iD마감, 999, 1)
    .setFormula(`=IF(${신청열문자}2="","",${신청열문자}2+15)`);
...
function _기능탭파싱등록(데이터, 제품명별접수번호) {
  ...
  // 헤더 인덱스 찾기 (유연 매칭 + 컬럼매핑 시트 커스텀 별칭)
  const idx = (내부키, 후보들) => {
    for (const c of _별칭합치기(내부키, 후보들)) {
...
function _접수대장기능수갱신(접수번호, 기능목록) {
  ...
  // 일정관리 갱신 (인공지능기능수만)
  const 일정시트 = ss.getSheetByName(SHEET.일정관리);
  const JD = 일정시트.getDataRange().getValues();
  const JH = JD[0];
  const jNo = JH.indexOf('접수번호');
  const j수 = JH.indexOf('인공지능기능수');
  for (let r = 1; r < JD.length; r++) {
    if (JD[r][jNo] === 접수번호) {
      if (j수 >= 0) 일정시트.getRange(r + 1, j수 + 1).setValue(기능목록.length);
      break;
    }
  }
}
...
function _Sheets에등록(건, 파일명) {
  ...
  if (신규) {
    ...
    const 일정값맵 = {
      ...
      '인공지능적용범위': 건.인공지능적용범위,
      '명세서작성방식':   건.명세서작성방식,
      '기타제출서류여부': 건.기타제출서류파일명 ? 'Y' : '',
      '보유인증':         건.보유인증,
      '인공지능기능수':   '',   // 기능탭 등록 후 갱신
    };
    일정시트.appendRow(일정H.map(h => 일정값맵[h] ?? ''));

    // 마감예정일 수식: 이미 초기설정에서 열 전체에 설정돼 있지만
    // appendRow로 새 행 추가 시 수식이 밀릴 수 있으므로 해당 셀만 재설정
    const 새행 = 일정시트.getLastRow();
    const iD신청 = 일정H.indexOf('신청일') + 1;
    const iD마감 = 일정H.indexOf('마감예정일') + 1;
    const 신청열문자 = columnLetter(iD신청);
    일정시트.getRange(새행, iD마감)
      .setFormula(`=IF(${신청열문자}${새행}="","",${신청열문자}${새행}+15)`);
  }
