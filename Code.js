...
const 체크항목정의 = [
  ...
  { id: 'C7', no: '3.1', 구분: '외부 인공지능 서비스 연동 확인', 방법: '서류·현장',
    항목: '외부 인공지능처리 연산체계와 연동된 경우 호출 로그나 API Key를 확인 가능한가?', 참조: '외부API모델' },
...
function _참조값생성(참조, 건, 기능목록) {
  switch (참조) {
    ...
    case '구현방식': {
      ...
      const 학습 = 기능목록.map(f => f['학습데이터사양'] || f['BaseModel']).filter(Boolean);
...
function _증적명세서Docs생성(ss, 건) {
  ...
  _명세표(body, [
    ...
    ['세부품명번호 / 물품식별번호', _모델번호요약(제품모델목록, 건)],
...
function _모델번호요약(제품모델목록, 건) {
  ...
  return rows.map((m, idx) => {
    ...
    return `${모델명}: 세부품명번호 ${세부} / 물품식별번호 ${물품}`;
  }).join('\n');
}
