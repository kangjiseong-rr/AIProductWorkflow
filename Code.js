...
function 수동직접등록_예시() {
  ...
  // 인공지능 기능 상세도 함께 등록 (구현방식 A~E 5종 모두 포함)
  AI기능상세등록(접수번호, [
    {
      ...
      구현방식: 'A',
      학습데이터사양: '자체 수집 흉부 X-ray 12만 건 (자체 소유, 병원 IRB 승인 데이터)',
      개발환경라이브러리알고리즘: 'Python 3.12, PyTorch 2.6.0, TorchVision, CNN 기반 객체 탐지 모델',
      연산자원요약: 'NVIDIA A100 40GB', 실행환경요약: 'Ubuntu 22.04, CUDA 12.1, PyTorch 2.6.0',
    ...
    },
    {
      ...
      출력: '폐렴 의심 등급, 중증도, 확신도',
      구현방식: 'B', BaseModel명칭: 'ResNet-50 v2 (torchvision)', 튜닝방법: '전이학습 + LoRA',
      튜닝데이터셋: '흉부 X-ray 3만 건 (오픈 데이터셋 CC-BY 라이선스), 전문의 3인 교차 검증',
      연산자원요약: 'NVIDIA A100 40GB', 실행환경요약: 'Ubuntu 22.04, CUDA 12.1, PyTorch 2.6.0',
    ...
    },
    {
      ...
      연산자원요약: '해당 없음 (외부 API 호출)',
      입력데이터설명: '탐지 결과 JSON, 임상 메모 텍스트',
      출력데이터설명: '판독 초안 텍스트',
    },
    {
      기능번호: 4, 기능명: '현장 촬영 즉시 스크리닝',
      인공지능역할: '이동형 촬영 장비에서 촬영 즉시 이상 여부 1차 스크리닝',
      입력: '촬영 즉시 획득한 흉부 X-ray',
      출력: '이상 의심 여부(정상/의심), 처리 지연시간',
      구현방식: 'D', 타겟HW_OS: 'NVIDIA Jetson Orin Nano 8GB, ARM Cortex-A78AE 6-core / Linux for Tegra',
      추론런타임: 'TensorRT 9.x, ONNX Runtime',
      연산자원요약: 'Jetson Orin Nano 내장 GPU', 실행환경요약: 'JetPack 6.0, Ubuntu 22.04 (ARM64)',
      입력데이터설명: '촬영 즉시 획득한 흉부 X-ray 프레임',
      출력데이터설명: '이상 의심 여부, 처리 지연시간(ms)',
    },
    {
      기능번호: 5, 기능명: '종합 판독 리포트 생성',
      인공지능역할: '기능 1~4의 결과를 종합해 최종 판독 리포트를 구성',
      입력: '기능 1~4 결과 JSON',
      출력: '종합 판독 리포트(PDF)',
      구현방식: 'E', 혼합구성설명: '자체 학습 모델(기능1) + 오픈소스 파인튜닝 모델(기능2) + 외부 API(기능3) 결과를 규칙 기반 로직으로 결합',
      모델별역할및입출력흐름: '1) 자체 탐지모델 결과 정규화 2) 분류모델 등급 매핑 3) 외부 API 요약문 삽입 4) 규칙 기반 종합판정',
      연산자원요약: 'CPU 4 vCPU (결합 로직만 수행, 개별 모델 자원은 기능1~3 참조)',
      입력데이터설명: '기능 1~4의 출력 JSON',
      출력데이터설명: '종합 판독 리포트(PDF), 종합 신뢰도 점수',
    },
  ]);
