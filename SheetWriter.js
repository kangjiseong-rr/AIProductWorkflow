...
function _테이블행추가(sheet, 값배열, 헤더행, 텍스트헤더목록) {
...
/** 기존 행은 건드리지 않고 신규 행에만 부여할 고정 순번의 다음 값. */
function _다음고정순번(시트) {
  if (!시트 || 시트.getLastRow() < 2) return 1;
  const H = 시트.getRange(1, 1, 1, 시트.getLastColumn()).getValues()[0]
    .map(v => String(v).trim());
  const i순번 = H.indexOf('순번');
  if (i순번 < 0) return 1;
  const 값들 = 시트.getRange(2, i순번 + 1, 시트.getLastRow() - 1, 1).getValues();
  const 기존최대 = 값들.reduce((최대, 행) => Math.max(최대, Number(행[0]) || 0), 0);
  const 기존데이터행수 = 시트.getLastRow() - 1;
...
function 접수번호한건삭제() {
  ...
  const 일치대상 = 대상시트명.filter(이름 => 조회[이름] && 조회[이름].행번호.length);
  if (!일치대상.length) {
    ui.alert(`운영 시트에서 일치하는 접수번호를 찾지 못했습니다: ${접수번호}`);
    return;
  }

  const 대표행 = 대장 && 대장.행번호.length
    ? 대장.첫행객체
    : 조회[일치대상[0]].첫행객체;
  const 기업명 = 대표행.기업명 || '-';
  const 제품명 = 대표행.제품명 || '-';
  ...
  try {
    ...
    const 최종일치대상 = 대상시트명.filter(이름 => 최종조회[이름] && 최종조회[이름].행번호.length);
    if (!최종일치대상.length) {
      throw new Error('최종 확인 중 일치 행이 사라져 삭제를 중단했습니다.');
    }
...
function _접수대장기능수갱신(접수번호, 기능목록) {
  ...
  // 일정관리의 인공지능기능수는 접수대장을 INDEX+MATCH 참조하므로 자동 반영됨 (별도 쓰기 불필요)
}
...
function _Sheets에등록(건, 파일명) {
  ...
  const 값맵 = {
    ...
    '담당자명': 건.담당자명, '담당자직급': 건.담당자직급, '담당자전화': 건.담당자전화,
    '담당자휴대전화': 건.담당자휴대전화, '이메일': 건.이메일,
    // 제품·서비스 정보
    '제품명': 건.제품명, '제품수': 건.제품수 || 1,
    '제공형태': 건.제공형태, '제공형태기타': 건.제공형태기타,
    '제품분류': 건.제품분류, '제품분류기타': 건.제품분류기타, '서비스도메인': 건.서비스도메인,
    '개요': 건.개요,
    '인공지능적용목적': 건.인공지능적용목적,
    '인공지능적용범위': 건.인공지능적용범위,
    '구조도파일명': 건.구조도파일명,
    ...
    // 기존 인증
    '보유인증': 건.보유인증, '기타인증명': 건.기타인증명,
    '인증서파일명': 건.인증서파일명, '인증비고': 건.인증비고,
    // 동의서·특기사항
    '열람이용동의여부': 건.열람이용동의여부,
    '개인정보수집이용동의여부': 건.개인정보수집이용동의여부,
    '개인정보3자제공동의여부': 건.개인정보3자제공동의여부,
    '최종신청동의여부': 건.최종신청동의여부,
    '특기사항': 건.특기사항, '접수일자': 건.접수일자,
  ...
  _테이블행추가(
    ...
    행값,
    H,
    ['접수번호', '사업자번호', '담당자전화', '담당자휴대전화']
  );

  // ── 일정관리 추가 ──
  // 하이브리드 싱크:
  //   · 신청정보(회사·연락처·제품 등) = 접수대장 INDEX+MATCH 수식 참조 (접수대장이 원본)
  //   · 상태·담당심사원 = 일정관리에서 직접 편집 (일정관리가 원본)
  //   · 마감예정일 = 접수일자 + 15 WD (주말·공휴일 제외) 수식
  const 일정등록결과 = _일정관리행추가(ss, 접수번호, {
    접수일자: 건.접수일자 || '',
    심사접수일: Utilities.formatDate(오늘, 'Asia/Seoul', 'yyyy-MM-dd'),
